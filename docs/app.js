...
function bindFilters() {
  const map = { search: (e) => { state.search = e.target.value; render(); }, sort: (e) => { state.sort = e.target.value; render(); } };
  const el = (id) => document.getElementById(id);
  if (el("search")) el("search").oninput = debounce((e) => { state.search = e.target.value; render(); }, 150);
